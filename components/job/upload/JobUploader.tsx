...
const JobUploader = ({ handleUpload }: Props) => {
    ...
    return (
        <div
            ...
            onClick={handleClick}
            className={cn(
                'p-4 bg-gray-50 border-2 rounded-2xl md:w-3xl shrink-0 md:h-70 w-full h-50 border-gray-100 border-dashed overflow-hidden hover:border-blue-100 duration-200 cursor-pointer',
                isDragIn && isValid && 'border-blue-100',
                isDragIn && !isValid && 'border-red-50'
            )}
        >
            <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                ...
                <div className="text-center">
                    <div className="text-[16px] text-gray-900 font-bold">
                        HTML 파일을 여기에 드래그하거나 클릭하세요
                    </div>
                    <div className="text-[14px] text-gray-400 font-bold">
                        Cmd+S로 저장한 채용공고 파일을 업로드하세요
                    </div>
                </div>
                <input
                    ref={inputRef}
